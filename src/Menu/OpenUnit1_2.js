...
export default class OpenUnit1_2 extends React.Component {
...
componentDidMount() {
  ...
  status1.on('value', snap => {
    this.setState({money12: snap.val()});
  });
...
{
  const number = 1
  firebasecc.child('Branch1').child('Washingmachine1_2').child('On_Off').set(number);
  ...
  <Text style={ { marginTop: 10, color: '#000000',fontSize: 21, fontFamily: Fonts.ltim} }>ยอดเงินคงเหลือ = {this.state.money12} บาท</Text>
  <View style={{
        marginTop: 50,
        alignItems:'center'
        }}>

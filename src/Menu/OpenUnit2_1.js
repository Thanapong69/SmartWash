...
export default class OpenUnit2_1 extends React.Component {
...
componentDidMount() {
  ...
  status1.on('value', snap => {
    this.setState({money13: snap.val()});
  });
...
{
const number = 1
firebasecc.child('Branch2').child('Washingmachine2_1').child('On_Off').set(number);
this.props.navigation.navigate('WashScreen');
  ...
  <Text style={ { marginTop: 10, color: '#000000',fontSize: 21, fontFamily: Fonts.ltim} }>ยอดเงินคงเหลือ = {this.state.money13} บาท</Text>
  <View style={{
        marginTop: 50,
        alignItems:'center'
        }}>

...
export default class Unit2_1 extends React.Component {
    ...
    ShowAlertDialog = () =>{
      ...
      onTick = () =>
      {
        ...
        firebasecc.child('Status').child('21').set(status_1);
        firebasecc.child("Reservations2_1").child(uid).update({uid:uid,name:name});
        if(this.state.money3<20) {
          alert("กรุณาเติมเงินด้วยครับ" );
          }else if(this.state.money3>=20){
          this.state.money3 = this.state.money3-20 
          firebase.database().ref(`/users/${uid}`).update({
          money : this.state.money3
        })
        this.state.count = this.state.count+1
      firebase.database().ref(`count1_3`).set(this.state.count)
   ...
   goNext (){
    this.props.navigation.navigate('OpenUnit2_1Screen');
   }
  ...
  render() {
    return (
      <View style={{
        ...
        <Text style={ styles.status }>คุณ {this.state.name}     <Text style={ { color: '#0000ff',fontSize: 18, fontFamily: Fonts.ltim} }>คงเหลือ = {this.state.money3} บาท</Text></Text>

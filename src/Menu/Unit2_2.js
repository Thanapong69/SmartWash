...
export default class Unit2_2 extends React.Component {
    ...
    componentDidMount() {
      ...
      const getcount = firebase.database().ref().child('count1_4');
    ...
    ShowAlertDialog = () =>{
      ...
      onTick = () =>
      {
        ...
        firebasecc.child('Status').child('22').set(status_1);
        firebasecc.child("Reservations2_2").child(uid).update({uid:uid,name:name});
        if(this.state.money4<20) {
          alert("กรุณาเติมเงินด้วยครับ" );
          }else if(this.state.money4>=20){
          this.state.money4 = this.state.money4-20 
          firebase.database().ref(`/users/${uid}`).update({
          money : this.state.money4
        })
        this.state.count = this.state.count+1
      firebase.database().ref(`count1_4`).set(this.state.count)
   ...
   goNext (){
    this.props.navigation.navigate('OpenUnit2_2Screen');
   }
  ...
  render() {
    return (
      <View style={{
        ...
        <Text style={ styles.status }>คุณ {this.state.name}     <Text style={ { color: '#0000ff',fontSize: 18, fontFamily: Fonts.ltim} }>คงเหลือ = {this.state.money4} บาท</Text></Text>
